            ...
            <div class="max-w-4xl mx-auto" v-else>
                ...
                <div class="bg-white rounded-lg shadow-sm border border-slate-200 p-6 mb-6">
                    <div class="flex items-center justify-between mb-4">
                        <h1 class="text-2xl font-semibold text-slate-900">Procesar Solicitud</h1>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        <div>
                            ...
                            <p class="text-slate-900">{{ store.records.user }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1">Usuario Aprobador</label>
                            <p class="text-slate-900">{{ store.records.manager }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1">Fecha de Solicitud</label>
                            <p class="text-slate-900">{{ store.records.created_at_format }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1">Fecha de Aprobación</label>
                            <p class="text-slate-900">{{ store.records.date_authorized_format }}</p>
                        </div>
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-2">Descripción de la Solicitud</label>
                        <div class="bg-slate-50 rounded-md p-3 border border-slate-200">
                            <p class="text-slate-800 leading-relaxed">{{ store.records.description }}</p>
                        </div>
                    </div>
                </div>
                <!-- Detalle de Suministros -->
                <transition name="fade" v-if="showRejectionReason">
                    <textarea placeholder="motivo de rechazo" v-model="store.reason_reject"
                        class="w-full px-3 pt-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </transition>
                <span v-if="showValidationReason" class="text-xs text-red-500">
                    La razón del rechazo debe tener al menos 10 caracteres.
                </span>
                <div class="flex justify-end mb-2">
                    <button @click="reject()"
                        class="px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700 disabled:bg-slate-300 disabled:cursor-not-allowed transition-colors duration-200 text-sm font-medium">
                        Rechazar
                    </button>
                </div>
                <div class="bg-white rounded-lg shadow-sm border border-slate-200 p-6">
                    <div class="flex items-center justify-between mb-4">
                        <h2 class="text-lg font-semibold text-slate-900">Suministros a Entregar</h2>
