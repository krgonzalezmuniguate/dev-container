<template>
    <div>
        ...
        <div class="min-h-screen bg-gray-50 p-6" v-if="!store.loadingRecords">
            <div class="font-extrabold text-4xl text-center mb-10">Solicitudes por procesar</div>
            <div class="max-w-7xl mx-auto">
                ...
                <div class="flex justify-center gap-3 mb-1">
                    ...
                    <Tooltip message="Recargar datos" position="top">
                        <button @click="store.index_to_manager()"
                    ...
                    <DataTable :data="store.records" :headers="store.headers">
                        ...
                        <template #actions="{ item }">
                            <div class="flex items-center justify-center gap-2">
                                ...
                                <Tooltip message="Procesar" position="top">
                                    <button @click="GoProcessing(item.id)"
                                        class="text-teal-700 hover:text-teal-900 p-1 font-bold"
                                        v-if="item.status === 'authorized'" title="Eliminar">
...
function GoProcessing(id) {
    router.push({ name: 'processing', params: { id: id } })
}
function downloadPdf(id) {
    pdfService.downloadSolicitud(id);
}
onMounted(() => {
    store.index_to_manager();
});
</script>
